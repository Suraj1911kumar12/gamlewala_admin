...
const PackageList = () => {
  ...
  useEffect(() => {
    GetData({ url: "pooja-package", token: token })
      .then((res) => {
  ...
  const columns = [
    ...
    },
    {
      title: "ITEMS",
      key: "elements",
      dataIndex: ["elements", "name"],
      ...columnSearchProps("elements"),
      render: (_, { elements }) => (
        <>
          {elements?.map((elements) => {
            return <Tag key={elements}>{elements.name}</Tag>;
          })}
        </>
      ),
    },
      ...
      render: (_, elem) => (
        <div className=" text-left px-2 py-1">
          <div className=" text-left">
            <div
              className={
                elem?.isActive ? "badge bg-success" : "badge bg-danger"
              }
            >
  ...
  const deleteItem = (id) => {
    DeleteData({ url: `pooja-package/delete/${id}`, token: token })
      .then((res) => {
        window.scrollTo(0, 0);
        if (res.data.status) {
          setAlert({
