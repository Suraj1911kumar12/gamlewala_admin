...
const EditPremiumEvent = () => {
  ...
  useEffect(() => {
    GetData({ url: `/premium-events/${params.id}`, token: token })
      .then((res) => {
        // console.log(res);
        if (res?.data?.status) {
          setDetails({
            ...
            segment: res?.data?.data?.category?.map((item) => item?.name),
  ...
  const update = async (e) => {
    ...
    EditData({ url: "premium-events/update", cred: credentials, token: token })
      .then((res) => {
        window.scrollTo(0, 0);
        setAlert({
          successStatus: true,
...
export default EditPremiumEvent;
